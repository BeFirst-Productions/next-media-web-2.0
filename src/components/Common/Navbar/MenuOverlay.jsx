...
export default function MenuOverlay({ isMenuOpen, setIsMenuOpen, origin = 'top-right' }) {
  ...
  // Prevent background scrolling and layout shifting when menu is open
  useEffect(() => {
  ...
  return (
    <div
      className={`fixed top-0 right-0 bottom-0 w-[96vw] lg:w-[94vw] z-200 bg-white ${expansionStyles.rounding} overflow-hidden transition-all duration-1200 ease-[cubic-bezier(0.76,0,0.24,1)] ${isMenuOpen ? 'opacity-100 pointer-events-auto translate-x-0' : 'opacity-0 pointer-events-none translate-x-12'
        }`}
      style={{
      ...
      <div className="flex flex-col h-full overflow-y-auto overflow-x-hidden">
        ...
        <Container className="pt-8 sm:pt-12 pb-4 shrink-0">
          <div className="flex justify-between items-center pr-2 sm:pr-4">
            <div className={`text-black/30 font-medium tracking-widest text-xs transition-all duration-700 ${isMenuOpen ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>
              NAVIGATION
            </div>
            <button
            ...
            >
              <span className="text-black text-4xl sm:text-5xl font-light transform transition-transform duration-300">
                ×
              </span>
            </button>
          </div>
        ...
        <Container className="flex-1 flex flex-col justify-center pb-12 sm:pb-16 mt-4 sm:mt-8">
          <div className="flex flex-col lg:flex-row items-center justify-between w-full h-full gap-12 lg:gap-8">
            ...
            {/* Left Nav List */}
            <div className="w-full lg:w-[45%] flex flex-col items-start space-y-4 sm:space-y-6 lg:space-y-8 pl-2 sm:pl-4">
              {navLinks.map((link, index) => (
                <Link
                  key={link.name}
                  href={link.href}
                  onClick={() => setIsMenuOpen(false)}
                  className={`group relative text-[28px] sm:text-4xl lg:text-[40px] xl:text-[56px] font-medium tracking-tight transition-all duration-700 ease-out ${isMenuOpen
                  ...
                  style={{
                    transitionDelay: isMenuOpen ? `${350 + index * 100}ms` : '0ms'
                  }}
                >
                  <span className="relative z-10">{link.name}</span>
                  <span className={`absolute -left-6 top-1/2 -translate-y-1/2 w-2 h-2 bg-[#2bc5ee] rounded-full transition-all duration-300 scale-0 group-hover:scale-100 ${isActive(link.href) ? 'scale-100' : ''}`}></span>
                </Link>
              ))}
            </div>
            ...
            <div className="w-full lg:w-[55%] flex justify-center lg:justify-end px-4 lg:px-0 mt-8 lg:mt-0 max-h-[400px] sm:max-h-[500px] lg:max-h-[600px] h-full overflow-hidden">
              <div
                className={`relative w-full aspect-4/3 lg:aspect-auto lg:w-[90%] lg:h-full rounded-[32px] overflow-hidden shadow-2xl transition-all duration-1000 ease-[cubic-bezier(0.23,1,0.32,1)] ${isMenuOpen ? 'opacity-100 scale-100 translate-x-0' : 'opacity-0 scale-90 translate-x-12'
                  }`}
                style={{
              ...
              >
                <img
                  ...
                  className={`w-full h-full object-cover transition-all duration-[2s] ease-out ${isMenuOpen ? 'scale-100 rotate-0 filter-none opacity-100' : 'scale-125 blur-lg opacity-50'
                    }`}
                />
        ...
        <Container className="shrink-0 pb-8 sm:pb-12 lg:pb-16 mt-2">
          <div className="w-full flex flex-col md:flex-row justify-between items-center text-black space-y-6 md:space-y-0 text-sm md:text-base lg:text-lg font-medium">
            ...
            {/* Phone Numbers */}
            <div className="flex flex-col space-y-2 lg:space-y-3 xl:-ml-12">
              <a href="tel:+971588984455" className="flex items-center space-x-3 hover:text-[#2bc5ee] transition-colors">
                ...
                <span>+971 58 898 4455</span>
              </a>
              <a href="tel:+971525162071" className="flex items-center space-x-3 hover:text-[#2bc5ee] transition-colors">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#2bc5ee" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" /></svg>
            ...
            <div className="flex items-center space-x-4 lg:space-x-5">
              {/* Instagram */}
              <a href="https://www.instagram.com/nextdigital.ae" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="w-[46px] h-[46px] rounded-full border-[1.5px] border-[#2bc5ee] flex items-center justify-center hover:bg-[#2bc5ee] hover:text-white transition-colors text-black">
              ...
              {/* LinkedIn */}
              <a href="https://in.linkedin.com/company/nextdm" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className="w-[46px] h-[46px] rounded-full border-[1.5px] border-[#2bc5ee] flex items-center justify-center hover:bg-[#2bc5ee] hover:text-white transition-colors text-black">
              ...
              {/* Facebook */}
              <a href="https://www.facebook.com/nextdms" target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="w-[46px] h-[46px] rounded-full border-[1.5px] border-[#2bc5ee] flex items-center justify-center hover:bg-[#2bc5ee] hover:text-white transition-colors text-black">
